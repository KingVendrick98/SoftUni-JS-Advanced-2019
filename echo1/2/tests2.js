...
describe(`Constructor tests`, function () {
    it(`Build new hotel throw error for name`, function () {
        let res = new SkiResort(`Some`)

        expect(() => res.build('', 55)).to.throw(Error, "Invalid input")
    })
    it(`Build new hotel throw error for ammount of beds`, function () {
        let res = new SkiResort(`Some`)

        expect(() => res.build('asd', -5)).to.throw(Error, "Invalid input")
    })
    it(`Build new hotel`, function () {
        let res = new SkiResort(`Some`)
    ...
    it(`Book hotel no free beds `, function () {
        let res = new SkiResort(`Some`);
        res.build('aaa', 1)
